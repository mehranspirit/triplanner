...
app.put('/api/trips/:id/collaborators/:userId', auth, async (req, res) => {
  try {
    const { role } = req.body;
    ...
    if (!trip.owner._id.equals(req.user._id)) {
      return res.status(403).json({ message: 'Only the trip owner can update collaborator roles' });
    }

    const collaborator = trip.collaborators.find(c => c.user._id.toString() === req.params.userId);
    if (!collaborator) {
    ...
    await logActivity({
      ...
      description: `Updated ${collaborator.user.name} to ${role} on trip "${trip.name}"`,
      details: {
        tripName: trip.name,
