...
const DepartureEventCard: React.FC<DepartureEventCardProps> = ({ event, thumbnail, onEdit, onDelete, onStatusChange }) => {
  ...
  const departureTimeDisplay = () => {
    // Get time directly from event object
    const hasDateTimeFields = event.date && event.time && 
                            event.date !== "" && event.time !== "";
    
    // console.log("DepartureEventCard: Attempting to display time with data:", {
    //   date: event.date,
    //   time: event.time,
    //   hasDateTimeFields,
  ...
  const handleCalendarClick = () => {
    ...
    try {
      ...
      calendarUrl.searchParams.append('text', `Departure from ${event.airport}`);
          ...
          <div className={cn(
            ...
            {isExploring ? (
              <>
                ...
                <span className="font-medium text-xs tracking-wide">DEPARTURE</span>
              </>
            ) : (
              <>
                <CheckCircle2 className="w-3 h-3" />
                <span className="font-medium text-xs">Departure</span>
              </>
            )}
          </div>
        </div>
          ...
          <div className="flex-grow space-y-2 relative">
            ...
            <div className="flex items-center text-sm space-x-2">
              ...
              <span className={cn(
                ...
                <span className="font-semibold">Departs:</span> {departureTimeDisplay()}
              </span>
            </div>
...
export default DepartureEventCard; 