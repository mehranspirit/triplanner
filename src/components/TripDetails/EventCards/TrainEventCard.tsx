...
import { TrainFront, Clock, MapPin, Edit, Trash2, Ticket } from 'lucide-react';
  ...
  return (
    <Card className="overflow-hidden flex flex-col h-full">
      <CardHeader className="p-0 relative">
        <img src={thumbnail} alt={event.trainOperator || 'Train'} className="w-full h-32 object-cover" />
      </CardHeader>
      <CardContent className="p-4 flex-grow space-y-2">
        ...
        {(event.carriageNumber || event.seatNumber) && (
            <p className="text-sm">Carriage: {event.carriageNumber || 'N/A'}, Seat: {event.seatNumber || 'N/A'}</p>
        )}
