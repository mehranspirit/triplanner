...
export const CollaboratorManagementModal: React.FC<CollaboratorManagementModalProps> = ({
  ...
  const isEditor = trip.collaborators.some(
    c => typeof c !== 'string' && c.user._id === user?._id && c.role === 'editor'
  );
      ...
      <div className="bg-white rounded-lg w-full max-w-2xl mx-4 max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="space-y-4">
            <h3 className="text-lg font-medium text-gray-900 sticky top-0 bg-white py-2">Current Collaborators</h3>
            {trip.collaborators.map((collaborator) => {
              if (typeof collaborator === 'string') return null;
              return (
                <div
